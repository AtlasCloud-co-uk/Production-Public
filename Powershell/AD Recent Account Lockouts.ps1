...
foreach ($DC in $DCList.DomainControllers)
    {
    $DC.Name
    Get-WinEvent -ComputerName $DC.Name -LogName Security -FilterXPath "*[System[EventID=4740 and TimeCreated[timediff(@SystemTime) <= 86400000]] ]" -ErrorVariable +errs -ErrorAction SilentlyContinue |
